...
function chatMe(msg)
{
    ...
    API.on(API.CHAT, function(data){
        if(data.message.indexOf('.') === 0){
            ...
            if(Funbot.misc.ready || Funbot.admins.indexOf(fromID) > -1 || API.getUser(data.fromID).permission > 1 || API.getUser(fromID).permission < 2){
                ...
                case "ban":
                        if(API.getUser(fromID).permission > 1 || Funbot.admins.indexOf(fromID) !== -1 || command[1] == API.getUsers().username){
    ...
    API.on(API.CHAT, function(data){
        ...
        for(var i = 0; i < Funbot.filters.beggerWords.length; i++){
            if(msg.indexOf(Funbot.filters.beggerWords[i].toLowerCase()) > -1 && Funbot.settings.beggerFilter){
                ...
                responses = ["@{beggar} Ask for fans again and i will ban you. ಠ_ಠ","Good idea @{beggar}!  Asking for fans... That's so yesterday!", "Guys @{beggar} asked us to fan him!  Lets all totally do it! ಠ_ಠ", "srsly @{beggar}? ಠ_ಠ", "@{beggar}.  Earning his fans the good old fashioned way.  Hard work and elbow grease.  A true american."];
    ...
    API.on(API.CHAT, function(data){
        ...
        if(msg.indexOf("i got to go") !== -1 || msg.indexOf("igtg") !== -1 || msg.indexOf("gtg") !== -1 || msg.indexOf("be back") !== -1 || msg.indexOf("going off") !== -1 || msg.indexOf("off to") !== -1 || msg.indexOf("brb") !== -1 || msg.indexOf("have to go") !== -1){
